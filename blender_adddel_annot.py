...
for node in obj.depth_first_traversal():
    if node.is_leaf:
        ...
        # TODO : convert to red/green
        # green if  node has both pred and groundtruth labels.
        # red if it only has one of them.
        box_color = colors[0]
        if hasattr(node, 'annot'):
            box_color = colors[len(node.annot)]
...
bpy.context.collection.objects.link(obj_wireframe)

# bpy.context.scene.objects.active = obj_wireframe
bpy.context.view_layer.objects.active = obj_wireframe
